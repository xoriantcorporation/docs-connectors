...
A typical use of the Invoke Rest API operation is to invoke Veeva Vault Rest API using the default GET method. By default, the GET methods do not send the payload in the request; the body of the HTTP request is empty. The other methods send the message payload as the body of your request.

After sending a request, the connector receives the response either in JSON or XML format depends on header *Accept* value provided, otherwise default is JSON. The payload is then passed to the next element in your application’s flow.
...
In General > Request > Query Parameters, click the plus icon (+) to add headers to the request. For example, add header names *HeaderName1* and HeaderName2, add header values HeaderValue1 and HeaderValue2. You can use DataWeave expressions, for example #[{'HeaderName1' : 'HeaderValue1', 'HeaderName2' : 'HeaderValue2'}].

==== URI Parameters
You configure a URI parameter when you want to use a placeholder, such as /objects/documents/{doc_id}, in the path of your request. To configure a URI parameter, type the placeholder enclosed in curly brackets in the Path field. Select URI Parameters, click +, and enter a name and value.
...
To send parameters in a POST request, In General > Request, select the POST method. In Body, construct the payload of the Mule message. Following is an examples:

. For *application/x-www-form-urlencoded* with the names and the values of the parameters to send.
+
----
...
+
In multipart/form-data, ensure the header *Content-Type* value contains boundry attribute with value same as dataweave output as shown, for example: *"multipart/form-data; boundary=abcdefg"*
+
----
	%dw 2.0
