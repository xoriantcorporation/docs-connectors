...
|*URI Parameters*|URI Parameters in key-value format to resolve value in a path, if there are no URI parameters in path keep it empty. Default is an *Empty List*.
...
|*Input*|By default, payload is a stream.
|*Output*|Invoke Rest API response will be in JSON/XML depends on header *Accept* provided. By default it is JSON format.
