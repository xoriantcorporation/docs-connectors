...
* Version (required): Veeva Vault API version.
* Client Id (required): The Client Id that need to send with each request header to Vault instance in format - \{company}-\{organization}-\{server}-\{component}.
* OIDC Profile Id: The OIDC Profile Id created on vault while configuring OAuth2.0/OIDC profile. See http://vaulthelp2.vod309.com/wordpress/admin-user-help/authentication-vault-security/configuring-oauth-2-oidc-profiles/[Configuring OAuth 2.0 / OpenID Connect Profiles]
...
* Authorization URL: Authorization URL Authorize to authorize the app for a user.
* Access Token URL: An access token url to retrieve access token for authentication.
