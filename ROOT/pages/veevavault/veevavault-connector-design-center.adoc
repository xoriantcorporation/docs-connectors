...
|link:#configure-oauth2-connection[*OAuth2 Connection*] |http://vaulthelp2.vod309.com/wordpress/admin-user-help/authentication-vault-security/configuring-oauth-2-oidc-profiles/[Configuring OAuth 2.0 / OpenID Connect Profiles]
|===
...
. Following is a configuration for *"1 - Basic Connection"*. See link:veevavault-connector-reference.adoc#veevavault-connector-connection-config[Connector Configuration] for "*2 - OAuth2 Connection*" configuration.
...
|*Client Id* |Enter Client Id in format *\{organisation}-{company\|team}* (for example: *"abcpharma-clinicalprod"*) to send with each request in header for tracking purpose. Before sending, the Client Id is prefix with *"mulesoft"* and suffix with *"client-vaultconnector"* internally as shown in e.g:   *mulesoft-abcpharma-clinicalprod-client-vaultconnector*. |Yes
|===
+
...
* Client Id (required): The Client Id that need to send with each request header to Vault instance in format - \{company}-\{organization}-\{server}-\{component}. 
...
* Version (required): Veeva Vault API version.
* Client Id (required): The Client Id that need to send with each request header to Vault instance in format - \{company}-\{organization}-\{server}-\{component}.
* OIDC Profile Id (required): The OIDC Profile Id created on vault while configuring OAuth2.0/OIDC profile. See http://vaulthelp2.vod309.com/wordpress/admin-user-help/authentication-vault-security/configuring-oauth-2-oidc-profiles/[Configuring OAuth 2.0 / OpenID Connect Profiles]
* Consumer Key (required): The consumer key for the Veeva Vault connected application registered on OAuth Service Provider.
* Consumer Secret (required): The consumer secret for the Veeva Vault connected application registered on OAuth Service Provider.
* Authorization URL (required): Authorization URL Authorize to authorize the app for a user.
* Access Token URL (required): An access token url to retrieve access token for authentication.
