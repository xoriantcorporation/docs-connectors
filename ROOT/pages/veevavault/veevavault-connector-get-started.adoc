...
* Invoke Rest API: Operation to invoke https://developer.veevavault.com/api/20.1/#documents[Veeva Vault Rest APIs], where a specific connector operation doesn't already exist. It's important to use bulk Rest APIs whenever possible, rather than single APIs so as to maintain API rate limits.
