= VeevaVault - Studio Configuration - Mule 4
...
Configure an input source for the connector such as an operation for the Veeva Vault connector, *HTTP Listener*, or *Scheduler*.
...
After configuring the Veeva Vault Connector for use in Studio, see the xref:veevavault/veevavault-connector-config-topics.adoc[Additional Configuration Information] 
