...
. Click *OK*
+
. Select *1 - Basic Connection* from Connection list. See link:veevavault-connector-reference.adoc#veevavault-connector-connection-config[Connector Configuration] for "*2 - OAuth2 Connection*" configuration.
+
...
|*Client Id* |Enter Client Id in format *\{organisation}-{company\|team}* (e.g: "abcpharma-clinicalprod") to send with each request in header for tracking purpose. The Client Id is prefix with *"mulesoft"* and suffix with *"client-vaultconnector"* internally. E.g : *mulesoft-abcpharma-clinicalprod-client-vaultconnector*
|===
+
